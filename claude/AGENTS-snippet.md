...
- Other filing skills for non-task-shaped work: `/ft-sidequest [--low|--med|--fut] [ID]` (park an idea or quick fix — tiny stub + PLAN line; flags skip the priority question, else one short ask; resume inline; lightest persistent filing), `/ft-starter-task [ID]` (rich-context filing, not yet ready to start; suggests an ID when omitted), `/ft-micro-task <ID>` (small task too light for the 4-phase ceremony), `/ft-file-followup [ID]` (≤50w note, no tasknote artifact; suggests an ID when omitted), `/ft-epic-discovery` and `/ft-close-epic <ID>` (multi-child code-sweep or feature epics). See `.flowtron/core/SPEC/tasknote-selection.md` §"When to use a tasknote (and when not to)" and `.flowtron/core/SPEC/epic.md` for the per-skill shape and lifecycle details.
...
ln -s ../../.flowtron/core/claude/commands/ft-sidequest.md      .claude/commands/ft-sidequest.md
...
ln -s ../../.flowtron/core/claude/skills/ft-sidequest         .claude/skills/ft-sidequest
