...
- Each PLAN.md task line carries a `[model]` segment (recommended primary labels: `[heavy]` for design/ambiguous/multi-file work, `[light]` for mechanical/well-scoped work; adopters may use any short token such as `opus`, `sonnet`, `haiku`, `gpt-5`, or their own names). The task runs end-to-end on the tagged model. If the loaded model doesn't match, surface the mismatch before continuing.
