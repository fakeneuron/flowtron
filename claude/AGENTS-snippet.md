...
- For bugs and unexpected behavior, `/ft-debug <TASK-ID>` is a peer to `/ft-task` that drives the standard 4-phase template with a hypothesis-first cadence (expected/observed → ranked hypotheses → minimal repro → bisect → fix → re-verify) embedded inside Phase 1 Discovery. Soft scaffolding, not a gate; same `--fast` semantics as `/ft-task`.
