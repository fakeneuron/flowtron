...
| **`--fast` / `-f`** | Trailing arg on `/ft-task`, `/ft-micro-task`, and `/ft-goal-task` (e.g. `/ft-task CORE-004 --fast`) | Operator force-skip: suppresses the 👁️ frontend visual-confirmation ask and the 📦 ready-to-commit signal trips, routing closure to autonomous commit. Does **not** weaken the drift carve-out — a `Re-scope`/`De-scope` verdict still fires the 🛠️ Phase 1→2 gate — and does **not** suppress a destructive-action banner (🗄️/▶️/📡/💻 escalation), which is a safety control on irreversible actions, not a routine signal. `/ft-spec` and `/ft-refactor` each have their own unrelated `--fast` that only skips a review pause (before writing a spec / before filing a refactor epic). | Routine runs where the conditional gates would fire but you want autonomous execution and accept owning visual confirmation yourself. |
| **`--unattended`** | Trailing arg on `/ft-task`, `/ft-micro-task`, `/ft-goal-task`, and `/ft-close-epic` (e.g. `/ft-task CORE-004 --unattended`); no short alias; composes with `--debug` / `--worktree` in any order | Operator-less posture: declares that **no operator is present to answer a gate**. Supersets `--fast`'s *autonomy*, not its delegations — never pass both. Two of the three `--fast` suppressions apply (📦 force-skip, 🛠️ no-op); the third, 👁️ suppression, is **not** inherited, because it transfers the visual check to an operator this posture declares absent. Plus exactly one added behavior: the six gates an operator-less run cannot answer (the 🛠️ Phase 1→2 drift carve-out, a destructive-action escalation 🗄️/▶️/📡/💻, a prerequisite ✋ `ACTION`, the Step 1.5 concrete-model STOP, a queued bundled in-📦 prompt, and the Phase 3 👁️ `CONFIRM` visual ask) **park** the tasknote — `status: blocked` plus a closed-set `park-reason:` code — instead of firing a banner into an empty session. A conversion removes a banner and never adds one; no new cue glyph, and the two-banner cap is untouched. It removes *pauses*, never *proof*: the paper-complete guard holds in all three parts, and a park is itself a stop. Contract in flowtron, runtime in the caller — flowtron ships no orchestrator. On `/ft-close-epic` the flag is **not** a `--fast` superset (that skill never accepted one) and the parent-flip is **deferred** instead of parked: the `.N` audit closes and commits, while the parent line and the cohort move stay operator-owned. | An orchestrator, scheduler, or headless session driving a flowtron task with nobody watching. Skip it whenever you are present: `--fast` already covers "do not ask me", and a park costs a resume. |
...
| **`--deep`** | Trailing arg on `/ft-epic-discovery` (e.g. `/ft-epic-discovery --deep`); no short alias, and the skill's only argument | Deep pre-pass: injects a `## 🧭 Deep Pre-pass` section into the `.1` Discovery tasknote scaffold and drives three discrete stages before Phase 1 Discovery begins — constitution → specify → clarify — each closing on a structured-ask review-and-confirm gate. Adds **staging ahead of the phase workflow**, not a phase: the 4-phase drive, the filed-children deliverable, and the banner count are untouched, and the default flow is byte-identical to the pre-`--deep` skill. Contract side: [`../SPEC/epic.md`](../SPEC/epic.md) §"Optional deep pre-pass". | High-uncertainty epics — where the shared design surface, contract impact, or per-child scope is genuinely unclear at filing time and three extra review rounds are worth the interruption. Skip it when the conversation has already crystallized the scope; the default flow compresses the same thinking into Phase 1 plus child filing. |
| **`/model <name>`** | Claude Code UI command, between sessions (e.g. `/model opus`) | Switches the active model so a task runs on its PLAN-line `[model]`. The post-closure candidate list signals the target model via emoji (`[heavy]🧠` / `[medium]🧩` / `[light]🔧` / `[xheavy]🔭`); `/model` is the operator's tool to act on that signal and pre-empt the Step 1.5 mismatch gate on the next hand-off. | On a hand-off whose next task's `[model]` isn't satisfied by the current model — a concrete mismatch, or a `[heavy]` task on a lighter-tier model — paste it before the next `/ft-*` invocation. |
...
- `--unattended` — ledger row (same `SPEC.md` + `SPEC/gates.md` sites; the posture is orchestrator-neutral and names no vendor, so it adds no new ledger surface of its own). Its executable interpretation is the shared `unattended-mode.md` fragment, wiring-layer by construction.
...
- `--deep` — ledger row (`SPEC/epic.md`, whose Section cell names the §"Optional deep pre-pass" bold label explicitly). Unlike the two flags below, its contract-layer site is already covered; the pre-pass itself is skill content, and the SPEC paragraph says so ("Contract details live in the skill").
...
`v5.21.0 · 2026-08-27 (dogfooded)`. See [`../docs/AGENT-COMPAT.md`](../docs/AGENT-COMPAT.md) §"Reading the cells" for the full `last-verified` format and update obligation.
