...
For starting an existing PLAN.md task (starter, follow-up line, or fresh), use `/task <TASK-ID>`. For small file + execute one-shots, use `/micro-task <TASK-ID>`. For filing a rich-context starter mid-flow, use `/starter-task <TASK-ID>`. For lightweight follow-up filings (no tasknote artifact), use `/file-followup <TASK-ID>`. For opening a new epic, use `/epic-discovery`. For closing one, use `/close-epic`. For migrating an existing project from a prior workflow system, use flowtron's `docs/MIGRATION.md` §2 manually — that path involves judgment calls and is not automated by this skill.
