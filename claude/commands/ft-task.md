...
- `/ft-task <TASK-ID> --unattended` — operator-less posture, for a caller with nobody present to answer a gate. A strict `--fast` superset (never pass both): every `--fast` suppression applies, and on top of that the five gates an operator-less run cannot answer — the 🛠️ drift carve-out, a destructive-action escalation, a prerequisite ✋ ACTION, the Step 1.5 concrete-model STOP, and a queued bundled in-📦 prompt — **park** the tasknote (`status: blocked` + a `park-reason:` code) instead of firing a banner into an empty session. It removes pauses, never proof: the paper-complete guard holds in full, and a park *is* a stop. Resume by re-invoking `/ft-task <TASK-ID>` with an operator present.
