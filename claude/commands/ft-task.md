---
description: Start a flowtron tasknote for the given task ID and drive it through the SPEC's 4-phase workflow. With `--debug`, adds a hypothesis-first cadence (expected/observed → ranked hypotheses → minimal repro → re-verify) for bugs and unexpected behavior.
...
The flags are orthogonal and compose in either order: `/ft-task <TASK-ID> --debug --fast` runs the hypothesis scaffolding without AskUserQuestion pauses, and the repro re-verify still runs.
