...
Invoke the `worktree-start` skill with `args="$ARGUMENTS"`. The skill performs the four mechanical steps from the convention (create `wt-<ID>` branch, `git worktree add` at the canonical sibling location, copy the live `_project/tasknote/<ID>.md` into the worktree, and print a precise handoff block with the `cd` target + "Clear your session, then use 🔧 /ft-task <ID>" line). 
