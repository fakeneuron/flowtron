...
- Each PLAN.md task line carries an optional `[model]` tag (`[opus]` / `[sonnet]`). The `/task` skill gates on it at start — if the active model doesn't match, it stops and surfaces the mismatch before continuing.
