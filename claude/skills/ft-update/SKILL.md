...
Report the added symlinks per platform (or "no new skills to wire"). Note: global/by-reference skills (`/ft-new-project`, `/ft-audit-repo`) are picked up by the user's agent-home wiring when desired, not per-project — do not add extra repo-scoped symlinks beyond each platform's snippet list. `/ft-update` is intentionally in the adopter subset; `/ft-release` is flowtron-self-only.
