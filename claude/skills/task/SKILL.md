...
- **Phase 4: Closure** — update affected docs/inventories, flip the PLAN.md line to `[x] **<TASK-ID>** — <description>. Completed YYYY-MM-DD.` (move it to the `Completed` section) and move the tasknote to `_project/tasknote/archive/<area>/<TASK-ID>.md` as a single closure write, then recap to the user. The recap = brief summary of what changed and key decisions, plus an optional verification request (one concrete thing for the user to check). Wait for confirmation.
