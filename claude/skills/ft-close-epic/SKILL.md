...
- Candidates: read the full PLAN.md task-line shape (including `[model]`) to know the recommended model. When *printing the list to the user*, emit only the emoji primary label (`[heavy]🧠` / `[light]🔧`) + "design vs mechanical" prose + shortname — drop the bare bracketed token from the visible suggestion output. Branches:
  ...
  - No misses + parent flipped + PLAN.md empty of queued tasks → suggest filing a new epic in this session before clearing (see copy-paste exception below).
...
- Copy-paste helper: emit a short visual cue that uses the same emoji primary label just printed for the chosen next-task candidate line (🔧 for [light] or light-appropriate tokens; 🧠 for [heavy] or heavy tokens). Put the invocation on its **own line as inline-code with no trailing period** — a trailing `.` collides with the `.N` epic-subtask grammar (`FE-132.3.`) and breaks copy/paste. Shape: a label line `🔧 Clear your session, then run:` followed by `` `/<next-skill> <ID>` `` alone on the next line (🧠 when the candidate carried 🧠). Never emit a literal `/clear then /model ...` instruction in the user-facing suggestion. **Exception for `/ft-file-followup` candidates**: surface these as "file in this session before /clear" instead (e.g., `✋ ACTION: file /ft-file-followup <NEW-ID> now (in this session) — it needs this session's context; then /clear.`). `/ft-file-followup` cannot self-bootstrap from PLAN.md reads the way other skills can. **Exception for new `/ft-epic-discovery` suggestions (no queued task in PLAN.md)**: same pattern — surface as "file in this session before clearing." A bare `/ft-epic-discovery` in a cleared session has no context to work from; the description must come from the current session's context. Shape: `✋ File in this session before clearing:` followed by `` `/ft-epic-discovery <one-paragraph description of the new epic>` `` on its own line.
