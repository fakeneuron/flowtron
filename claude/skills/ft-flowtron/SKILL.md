...
| `/ft-audit-repo` | First-contact holistic repo audit — Repo Map before judging, thin capped sweep, 3–5 themes (won't-fix tradeoffs + done-signals), milestone plan filed as flowtron epics, delegation to the focused audit skills. Stack-neutral, read-only, no fork — run from the submodule path. |
