...
- **Concrete mismatch** → STOP. Read the same two in parallel, then follow the "Mismatch" branch. **When `unattended-mode = true`**, take `<SKILL_DIR>/unattended-mode.md` §"Pre-scaffold stops" instead — scaffold the tasknote with `status: blocked` + `park-reason: model-mismatch — …` and halt, rather than offering the two-path ask.
...
- **Fire branch** (privileged-ops signal hits OR bundled in-📦 prompt queued) — run its **bundled-approval motion**: surface the 📦 gate and wait for commit-go (SPEC/gates.md §"Accepted gate replies"). The 🏁 marker — carrying a 1-2 sentence accomplishment summary — plus next-move and copy-paste land only in the later post-commit response.
