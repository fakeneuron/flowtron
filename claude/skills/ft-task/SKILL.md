---
name: ft-task
description: Start a flowtron tasknote and drive it through the SPEC's 4-phase workflow. Use when the user asks to start a full tasknote for a normal-sized, multi-step, or design-tradeoff-bearing task — and, with `--debug`, when the user asks to debug a bug, regression, flaky behavior, or other unexpected behavior whose root cause is not yet known (hypothesis-first cadence — expected vs observed → ranked hypotheses → minimal repro → re-verify). Invoke with the task ID as args (e.g., args="CORE-004", "CORE-004 --debug", or "CORE-195.2 --debug --fast"); `--unattended` runs the operator-less posture, parking at any gate instead of firing a banner into an empty session. Reads SPEC.md, scaffolds the tasknote from the template, runs Phase 1 Discovery, then continues conversationally through phases 2-4 and the post-closure protocol.
---
...
`unattended-mode` is the operator-less posture — the caller declares that **nobody is present to answer a gate**. It supersets `--fast`'s *autonomy*, not its delegations: 📦 force-skip and 🛠️ no-op are inherited, but 👁️ suppression is not (it hands the visual check to a present operator, and there is none), so the 👁️ ask converts to a park. **When `unattended-mode = true`, Read `<SKILL_DIR>/unattended-mode.md` now** (and `<SPEC_DIR>/blocked.md` alongside it, since every conversion writes a park); it carries the park recipe, the per-runner conversion map, the pre-scaffold stop split, and what the posture never relaxes. Behavioral branches reference it at Step 1.5, Step 2, Step 4, Step 5, and Step 6. Contract: SPEC/gates.md §"`--unattended` operator posture".

The flags are orthogonal and compose: `--debug --fast` runs the hypothesis scaffolding without AskUserQuestion pauses, and the Phase 3 repro re-verify still runs (it is not a signal trip `--fast` may suppress). `--debug --unattended` does the same and additionally parks rather than asking, in either order.
...
- **Concrete mismatch** — a concrete tag differs from the active model → STOP. Read `<SPEC_DIR>/model.md` + `<SKILL_DIR>/step-1.5-model-edge.md` in parallel, then follow the "Mismatch" branch. **When `unattended-mode = true`**, take `<SKILL_DIR>/unattended-mode.md` §"Pre-scaffold stops" instead — scaffold the tasknote with `status: blocked` + `park-reason: model-mismatch — …` and halt, rather than offering the two-path ask.
...
- **Foreign-dirt gate (paper-complete guard).** Before any scaffold / promote / resume writes, run `git status --porcelain`. If non-empty: **STOP**, surface the dirt list, ask the operator to commit / stash / discard themselves, then re-invoke. Do not auto-clean. See SPEC §"Paper-complete guard". **`--unattended` does not relax this** — it terminates and writes nothing, in the machine-readable stop shape at `<SKILL_DIR>/unattended-mode.md` §"Pre-scaffold stops". The same applies to the archive-collision and in-flight-tasknote checks below.
...
- **Fire branch** (privileged-ops signal hits OR bundled in-📦 prompt queued) — surface the bundled 📦 ready-to-commit gate and **wait** for commit-go (closed set: `"commit"`/`"go"`/`"yes"` — SPEC/gates.md §"Accepted gate replies"). Do **not** emit 🏁, next-move, or the copy-paste line in this turn. After commit lands and passes the deliverable-covering check, the 🏁 marker (with 1-2 sentence accomplishment summary) + next-move + copy-paste follow in that later response.
