...
- Copy-paste helper: emit a short visual cue that uses the same emoji primary label just printed for the chosen next-task candidate line (🔧 for [light] or light-appropriate tokens; 🧠 for [heavy] or heavy tokens). Put the invocation on its **own line as inline-code with no trailing period** — a trailing `.` collides with the `.N` epic-subtask grammar (`FE-132.3.`) and breaks copy/paste. Shape: a label line `🔧 Clear your session, then run:` followed by `` `/ft-task <next-ID>` `` alone on the next line (🧠 when the candidate carried 🧠). Never emit a literal `/clear then /model ...` instruction in the user-facing suggestion. The emoji on the candidate line + the matching cue glyph are the stable, agent-agnostic pattern (per SPEC §"Post-closure protocol" step 3).
