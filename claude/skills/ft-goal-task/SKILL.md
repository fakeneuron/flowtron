...
- These are checked **once, after the loop converges** (Step 6), via a single emphasized `👁️ **CONFIRM**` prose ask (own line, bold label — see SPEC/gates.md §"Emphasized inline ask shape") — never inside the cycle (the loop runs with `--fast` semantics, which suppresses the per-cycle 👁️ ask; see Step 5).
...
1. **One-time taste checks.** If Phase 1 split any criteria into `### 👁️ One-time visual checks`, surface them now as a single emphasized `👁️ **CONFIRM**` prose ask — own line, blank-line isolated, bold label per SPEC/gates.md §"Emphasized inline ask shape" (suppressed when `fast-mode = true`, where the operator owns visual confirmation). This is the *only* 👁️ ask in the whole run. **When `unattended-mode = true`** the suppression does not apply — the posture does not inherit that delegation — so instead of surfacing or suppressing the ask, **park** here with `park-reason: visual-confirm — <the split-out criteria, and where to look>` per `<UNATTENDED>` §"Conversion map" and stop before Phase 4. A converged loop is not a finished task while a taste criterion is unchecked.
