...
2. **Execute (Phase 2).** On the **first** cycle, do the pattern survey (neighboring code / existing shape) before the first edit. Each cycle makes the **minimal** change targeting the currently-failing verify command(s). Keep edits tightly scoped.
3. **Verify (Phase 3).** Run the Acceptance verify commands (targeted — only what the change could affect, or the full set near convergence). Also run lint/type-check on changed code.
4. **Commit-or-retry.**
