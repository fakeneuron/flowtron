...
> Lazy-loaded SKILL fragment. Loaded by `micro-task` SKILL.md Step 1.5 when the concrete-mismatch, category under-tier, or legacy-entry branch fires. See `claude/skills/ft-micro-task/SKILL.md` for the always-loaded core dispatch and the satisfied-match path, and `SPEC/model.md` §"Category-vs-concrete matching" for the tier ladder + rule.
