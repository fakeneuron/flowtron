---
name: ft-audit
description: Ruthless principal-engineer code audit — 5 passes (Security · Idioms · Hygiene · Orphans · Doc drift), capped at 5 findings each, writes prioritized tickets to `.flowtron/PLAN.md`. Use when the user asks for a general code audit or review with no specific domain named (security/backend/frontend/etc. — prefer the focused `ft-audit-*` skill when one is named). Stack-neutral scaffold; adopters fork into `.claude/skills/audit/` and fill in rubric / verification gates / per-pass examples for their stack. See `docs/MIGRATION.md` §1.2.1.
---
...
4. **No code changes**, no source edits, no opening files for fixes. Tickets only — actual fixes happen in separate `/ft-task` cycles. One exception: the skip-the-tasknote carve-out below.
...
- **Write tickets, not fixes.** `.flowtron/PLAN.md` gets updated (§5 above). Source files do NOT — any code change needs a separate explicit user request. Do not open files in edit mode for fixes, do not run formatters, do not "fix while I'm in here." **Exception:** the §5 trivial-fix carve-out — skip-the-tasknote-sized fixes may be applied inline and recorded under `## Completed`.
