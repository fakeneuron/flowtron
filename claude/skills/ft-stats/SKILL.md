...
| `model` | `heavy` \| `medium` \| `light` \| `fable` \| `opus` \| `sonnet` \| `other` \| `legacy` |
...
`fable`/`opus`/`sonnet` are always shown if present in the data.
...
## Model distribution
<Section A table (expanded for current tiers: heavy/medium/light + fable/opus/sonnet + other/legacy)>
