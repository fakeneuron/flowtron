...
- **Release-gate only, like D and F–J.** The `drift` CI job runs the release-context-free subset (A, B, C, E, N) per `docs/CONVENTIONS.md` §"GitHub Actions CI"; promoting K there is a separate call, not implied by minting it.
...
- **Coverage is the eight lifted checks, and only those.** `docs/CONVENTIONS.md` §"GitHub Actions CI" deliberately keeps SOP currency, the README task counter, installed-surface policy, self-wiring, Pairs D, F–K and M, and Pair A's content half in §7.1 alone. Pair L claims nothing about them; a new step added to the `drift` job needs a new mapping row here, or it ships unbound.
- **Release-gate only, like D, F–K and M.** Promoting L into the `drift` job would make the job the sole judge of its own fidelity to a source it would then also be lifting — recorded here so a later reader does not read its absence as an oversight.
