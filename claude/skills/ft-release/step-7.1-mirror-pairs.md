...
- **Release-gate only, like D and F–I.** The `drift` CI job runs the release-context-free subset (A, B, C, E, J, M, N, O) per `docs/CONVENTIONS.md` §"GitHub Actions CI"; promoting K there is a separate call, not implied by minting it.
...
- **`echo`-argument text is stripped from both sides.** The adaptation delta lives in the findings messages, and Pair C's CI copy names `.flowtron/` inside one (`"skills write templates one level under .flowtron/"`). Stripping quoted `echo` arguments compares what each check *reads* rather than what it *says*. Every one of the eight steps still yields at least one path after the strip, so no step is silently vacuous — check that property when adding a step.
- **Coverage is the eleven lifted checks, and only those.** `docs/CONVENTIONS.md` §"GitHub Actions CI" deliberately keeps SOP currency, the README task counter, installed-surface policy, self-wiring, Pairs D, F, G, H, I, and K, and Pair A's content half in §7.1 alone. Pair L claims nothing about them; a new step added to the `drift` job needs a new mapping row here, or it ships unbound.
...
Must print nothing. `NO CANDIDATES LITERAL` is a filer that cites the module but never emits the line it defines; `NO LABELED MIRROR` is a citation with no label in the fixed shape `` mirror of `SPEC/unattended-candidacy.md` §"<section>" ``; `STALE LABEL` is a label whose section is no longer a `## ` heading in the module. Fix the first two on the filer, in the module's §"Surfaces and mirrors" shape; fix a `STALE LABEL` by updating the citation to the heading's current text — never by renaming the heading back.
...
- **The label resolves to a heading, not just to the file.** The K1 idiom: `grep -qF --` on `## <section>` so `/` and `.` in a title are literal and a title mentioned in prose cannot satisfy the check. Titles are matched as prefixes of the heading line, so `§"Three postures"` resolves against `## Three postures` and would still resolve if the heading grew a suffix.
- **`bad=` survives the inner loop** because the section list is fed by process substitution (`done < <(…)`), not a pipe — a piped `while` runs in a subshell and would drop the flag, reporting a `STALE LABEL` and passing anyway. Same family as Pair L's escape note: a matcher that fails open reports clean instead of reporting the truth.

**Pair O — filing runners ↔ `SPEC/tasknote-selection.md` §"Filing commits".** Five skill bodies commit their own filing at hand-off — `/ft-file-followup` (default, `--park`, `--starter`), `/ft-audit`, and `/ft-refactor` — under one contract that lives once in §"Filing commits": a pre-check that sets `auto-commit = `, a post-stage read of the **whole** index (`git diff --cached`, no pathspec, not `--quiet`) so a foreign hunk is seen before the commit publishes it, and a citation naming the section that owns the rule. Each runner restates the motion in its own step shape, and nothing bound them: [[CORE-563]] minted the contract and four runners, [[CORE-591]] patched `/ft-refactor`'s pre-check only, and the runner sat without a post-stage read until [[CORE-593]] — three commits over one rule, each leaving a different runner short, with no detector between them. Pair N guards the same class for `[unattended]` candidacy and is scoped to `SPEC/unattended-candidacy.md` by construction; every other pair is roster- or flag-derived and blind to a skill body's *procedure*.

Trigger is the `auto-commit = ` literal, not an allowlist of runners — a skill that starts setting the flag is bound the day it does. `claude/skills/ft-release/` is excluded because this pair is the check's home, not a filer. The citation half resolves the path each runner *names* rather than a fixed file, so a module split that moves the section (the one [[CORE-595]] is filed to make) repoints cleanly: a runner still citing the old file fails `STALE CITATION`, one citing the new file passes with no edit here:
...
Must print nothing. `NO POST-STAGE DIFF` is a runner that sets `auto-commit = ` but never reads the staged index before committing — every `git diff --cached` it carries is the pre-check's `--quiet` probe; `NO RESOLVING CITATION` is a runner with no path-bearing `` `SPEC/<module>.md` §"Filing commits" `` citation (a bare `per §"Filing commits"` shorthand is fine *alongside* one, not instead of it); `STALE CITATION` is a citation whose named module no longer carries a `## Filing commits` heading. Fix the first two on the runner, in the shape `ft-file-followup/starter-mode.md` uses (a `git add` / `git diff --cached` / `git commit` fence plus a **Post-stage verification** paragraph — [[CORE-593]] is the worked example); fix a `STALE CITATION` by repointing the citation to the module that now owns the heading — never by restoring the heading to satisfy the check.
...
- **Three citation shapes resolve, on purpose.** Runners write the citation as `` `SPEC/x.md` §"…" `` (backticked), `SPEC/x.md §"…"` (bare), or `](../../../SPEC/x.md) §"…"` (a markdown link — `ft-task/unattended-mode.md`'s shape, not a filer today). The `[`)]?` class absorbs the closing backtick or paren, and the `SPEC/`-anchored path skips any `../` prefix, so all three land on a repo-root-relative path `grep` can open. A fourth shape needs a class edit here, not a new pair.
- **The heading match is anchored, unlike Pair N's.** N matches `## <section>` as a prefix anywhere in the line so a suffix-grown heading still resolves; O anchors `^## Filing commits` because the title is fixed by the contract and a mention in prose must not satisfy it. Same K1 reasoning, tighter because the target is one known string.
- **`2>/dev/null` on the heading grep is deliberate.** A citation naming a module that does not exist yet (a split filed but not landed) should report as `STALE CITATION`, not as a `grep` stderr line followed by the same finding — one line per miss, like every pair above.
