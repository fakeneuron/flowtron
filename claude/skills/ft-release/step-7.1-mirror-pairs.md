...
- **Release-gate only, like D and F–I.** The `drift` CI job runs the release-context-free subset (A, B, C, J, M, N, O, P) per `docs/CONVENTIONS.md` §"GitHub Actions CI"; promoting K there is a separate call, not implied by minting it.
...
- **Unparseable stamp → exempt, and that is a known gap.** Two legacy notes (CORE-410.3, CORE-577.6) still hold the template's `YYYY-MM-DD` placeholder, and a note with no date cannot be placed against the floor. A future closure that leaves the placeholder unfilled is therefore invisible here — a stamp miss, a different class from tick-through, and it sits with CORE-610.3's pre-archive gate rather than this pair. Do not "fix" it by treating an unstamped note as post-floor: that retroactively binds the two legacy files.
