...
**Two kinds of pair.** Ten pairs — A, B, C, H, J, M, N, O, P, Q — are lifted into the CI `drift` job, and for those the **`ci.yml` step is the shell**: the entry below is a catalogue line (purpose · `Reads:` · `CI step:` · what a finding means and how to fix it), the design notes sit as `#` comments inside the step, and Pair L binds the `Reads:` line to what the step actually reads ([[CORE-631.2]]; before it, both surfaces carried the shell and the directory budget paid for it twice). The other pairs — D, F, I, K, L — need release context or judgment and keep their shell here. `/ft-release` §6.1 has already gated the cut on CI for HEAD, but this cut's own edits (version pins, doc sweeps) can mint a Pair H or Pair Q miss before commit, so run the ten locally now, on the working tree, under the CI shell:
...
for p in A B C H J M N O P Q; do
  ci=$(ci_paths "Pair $p ")
  rd=$(awk -v p="^[*][*]Pair $p " '
        $0~p                     {ins=1; next}
        ins && /^[*][*]/         {ins=0}
