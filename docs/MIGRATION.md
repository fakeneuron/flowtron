...
To install the scaffold, fork the **whole directory** — `SKILL.md`, its lazily-loaded `scaffold-bootstrap.md` sibling, and the `passes/` library:
...
- **Adopter-subset smoke.** Enumerate what §1.2 wired and invoke each in a fresh session with your coding agent's wired entry point — `/ft-*` for Claude Code, Cursor, or Grok, `/skills` or `$ft-*` for Codex, or a conversational prompt for contract-only agents:
  ...
  Confirm every slug it prints appears in your agent's roster (v1.0+ additions; symlinks added in §1.2; worktree pair added in CORE-215.5; `/ft-spec` added in CORE-352.3; `/ft-refactor` added in CORE-463.5).
...
- **Final pin verification.** `git -C .flowtron/core describe --tags` shows the pinned version recorded at the start (e.g., `v5.26.0`). A mismatch means the submodule drifted off the pin during migration.
...
| `ft-goal-task` | v5.27.0 | `/ft-task <ID> --loop` — same goal-loop contract (`SPEC/loop.md`: verify-command rule, per-cycle relevance gate, `loop-max`, `## 🔁 Iterations`), now a flag on the core runner backed by the `step-5-loop-mode.md` lazy fragment. `--worktree` was not carried over: run Phase 1, `/ft-worktree-start <ID>`, then `/ft-task <ID> --loop` in the worktree. Existing loop tasknotes (`loop: true`) resume under the flag unchanged |
