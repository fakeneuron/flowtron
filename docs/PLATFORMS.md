...
|---|---|---|---|
| **Effort / thinking level** | Model selection at session start — grok-3 (default, faster) vs grok-4 (extended thinking, heavier) per xAI model options | Maps to the `[heavy]` / `[light]` PLAN-line tokens. Heavier reasoning suits Discovery-heavy or cross-cutting tasknotes; lighter suits mechanical edits. | Match the running model to the task's `[model]` token. Switch before invoking the skill if the task's `[model]` differs from the current session model. |
