...
- **`commands/`**: 11 `.md` slash-command stubs (`ft-task.md`,
  ...
  plus `ft-new-project.md`, `ft-release.md`, `ft-update.md`)
- **`skills/`**: 11 `SKILL.md` skill bodies (one per command), some with
  lazy-load fragments (`ft-task/step-*.md`, including `step-4-debug-mode.md` and `step-5-loop-mode.md`; `ft-file-followup/step-0-flags.md`, `park-mode.md` and `starter-mode.md`; `ft-audit/scaffold-bootstrap.md`) or sibling libraries (`ft-audit/passes/*.md`)
...
**Last verified:** `v5.28.0 · 2026-09-14 (dogfooded)`
...
| **Structured ask** | No multi-option question primitive appears in Codex's slash-command reference, and every `codex/skills/*/SKILL.md` wrapper instructs a **prose ask** where the source skill asks for a structured one — via `codex/AGENTS-snippet.md` §"Translation rules", which holds that rule once for all 11 wrappers. The 2026-06-01 CORE-258 dogfood saw operator cues render legibly in conversation, with the UPPERCASE labels as the durable fallback. | Realizes Phase 1 clarification asks and other discrete decision points. The ask degrades to prose with no contract impact — the contract names the *operation*, not the primitive. | Phrase multi-option forks so they read cleanly as prose. Update this row if a Codex structured prompt is observed — absence here reflects the documented surface, not a vendor guarantee. |
...
**Last verified:** `v5.22.0 · 2026-08-29 (dogfooded; skipped @ v5.28.0)`
