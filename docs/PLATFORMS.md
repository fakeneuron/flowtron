...
| **Unattended posture (`--unattended`)** | Trailing `--unattended` (no short alias) on `/ft-task`, `/ft-micro-task`, `/ft-goal-task`. Same availability as `--fast`: the flag and the shared `unattended-mode.md` lazy fragment live in the loaded skill body, and Grok resolves the fragment relative to that body. | Operator-less posture — a strict `--fast` superset (never pass both). Everything `--fast` suppresses stays suppressed, and on top of that the five gates a run with no operator cannot answer (🛠️ drift carve-out, destructive-action escalation, prerequisite ✋ ACTION, Step 1.5 concrete-model STOP, queued bundled in-📦 prompt) **park** the tasknote — `status: blocked` plus a closed-set `park-reason:` code — instead of firing a banner into an empty session. Removes pauses, never proof: the paper-complete guard is untouched. | An orchestrator or headless session driving a flowtron task with nobody watching. Skip it when you are present — `--fast` already covers "do not ask me", and a park costs a resume. |
...
| **Unattended posture (`--unattended`)** | Trailing `--unattended` (no short alias). Same two-step routing as `--fast` — the SOP names *unattended mode* as a neutral primitive and records `--unattended` as its Claude spelling; the canonical body carries the literal parse and the shared `unattended-mode.md` fragment. The `ft-micro-task` / `ft-goal-task` wrappers route straight to the canonical body. | Operator-less posture — a strict `--fast` superset (never pass both). Everything `--fast` suppresses stays suppressed, and on top of that the five gates a run with no operator cannot answer (🛠️ drift carve-out, destructive-action escalation, prerequisite ✋ ACTION, Step 1.5 concrete-model STOP, queued bundled in-📦 prompt) **park** the tasknote — `status: blocked` plus a closed-set `park-reason:` code — instead of firing a banner into an empty session. Removes pauses, never proof: the paper-complete guard is untouched. | An orchestrator or headless session driving a flowtron task with nobody watching. Skip it when you are present — `--fast` already covers "do not ask me", and a park costs a resume. |
...
| **Unattended posture (`--unattended`)** | Trailing `--unattended` (no short alias) on `/ft-task`, `/ft-micro-task`, `/ft-goal-task`. The flag and the shared `unattended-mode.md` lazy fragment live in the loaded skill body. | Operator-less posture — a strict `--fast` superset (never pass both). Everything `--fast` suppresses stays suppressed, and on top of that the five gates a run with no operator cannot answer (🛠️ drift carve-out, destructive-action escalation, prerequisite ✋ ACTION, Step 1.5 concrete-model STOP, queued bundled in-📦 prompt) **park** the tasknote — `status: blocked` plus a closed-set `park-reason:` code — instead of firing a banner into an empty session. Removes pauses, never proof: the paper-complete guard is untouched. | An orchestrator or headless session driving a flowtron task with nobody watching. Skip it when you are present — `--fast` already covers "do not ask me", and a park costs a resume. |
