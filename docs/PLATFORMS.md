...
|---|---|---|
| **Claude Code** | Wiring layer + contract layer. Nine tasknote skills (`/ft-task` — which carries debug mode behind `--debug` — `/ft-starter-task`, `/ft-micro-task`, `/ft-file-followup` (which carries park mode behind `--park`), `/ft-epic-discovery`, `/ft-close-epic`, `/ft-goal-task` — which carries worktree handoff behind `--worktree` — `/ft-spec`, `/ft-refactor`) plus two thin worktree utilities (`/ft-worktree-start`, `/ft-worktree-end`) drive the SPEC's 4-phase workflow inline (or accelerate independent epic children); the parameterized `/ft-audit <domain>` skill runs the 5-pass recipe over seven domains and `/ft-audit-repo` runs the first-contact holistic recipe; standalone skills `/ft-new-project`, `/ft-release`, `/ft-flowtron`, `/ft-stats`, `/ft-audit-context`, `/ft-update` follow their own recipes. | `claude/` — `AGENTS-snippet.md` + `commands/*.md` + `skills/*/SKILL.md` (+ lazy fragments). Adopter installs follow the subset policy below and the executable commands in `claude/AGENTS-snippet.md` §"One-time symlink wiring". |
...
- **`commands/`**: 19 `.md` slash-command stubs (`ft-task.md`,
  ...
  `ft-epic-discovery.md`, `ft-close-epic.md`, `ft-goal-task.md`, `ft-spec.md`, `ft-refactor.md`, `ft-worktree-start.md`, `ft-worktree-end.md`, the two `ft-audit`-family skills (`ft-audit.md`, `ft-audit-repo.md`),
...
**Last verified:** `v5.19.0 · 2026-08-24 (dogfooded)`
...
| **Unattended posture (`--unattended`)** | Trailing `--unattended` (no short alias). Same two-step routing as `--fast` — the SOP names *unattended mode* as a neutral primitive and records `--unattended` as its Claude spelling; the canonical body carries the literal parse and the shared `unattended-mode.md` fragment. The `ft-micro-task` / `ft-goal-task` / `ft-close-epic` wrappers route straight to their canonical bodies. | Operator-less posture — a strict `--fast` superset (never pass both). Everything `--fast` suppresses stays suppressed, and on top of that the five gates a run with no operator cannot answer (🛠️ drift carve-out, destructive-action escalation, prerequisite ✋ ACTION, Step 1.5 concrete-model STOP, queued bundled in-📦 prompt) **park** the tasknote — `status: blocked` plus a closed-set `park-reason:` code — instead of firing a banner into an empty session. Removes pauses, never proof: the paper-complete guard is untouched. On `/ft-close-epic` it is not a `--fast` superset (that skill never took one) and the parent-flip is **deferred** rather than parked — the `.N` audit closes and commits, the parent line stays open, and the cohort move stays operator-owned. | An orchestrator or headless session driving a flowtron task with nobody watching. Skip it when you are present — `--fast` already covers "do not ask me", and a park costs a resume. |
...
| **Deep pre-pass (`--deep`)** | Trailing `--deep` on `ft-epic-discovery` (no short alias); named in that wrapper's `description:`. Same direct routing as `--park` — the wrapper delegates to the canonical body, which owns the flag inline in its own steps, so there is no lazy fragment and no two-step SOP hop. | Stages three discrete rounds — constitution → specify → clarify — into a `## 🧭 Deep Pre-pass` section of the `.1` tasknote before Phase 1 Discovery begins, each closing on a review-and-confirm gate. Staging ahead of the phase workflow, not a phase; the default flow is unchanged. | High-uncertainty epics where the shared design surface or per-child scope is genuinely unclear at filing time. The three stage gates are structured asks, and `codex/AGENTS-snippet.md` §"Translation rules" degrades those to a **prose ask** for every wrapper — so expect three conversational review-and-confirm rounds rather than option pickers. Budget for that when weighing the flag's interruption cost. |
...
| **Structured ask** | No multi-option question primitive appears in Codex's slash-command reference, and every `codex/skills/*/SKILL.md` wrapper instructs a **prose ask** where the source skill asks for a structured one — via `codex/AGENTS-snippet.md` §"Translation rules", which holds that rule once for all 19 wrappers. The 2026-06-01 CORE-258 dogfood saw operator cues render legibly in conversation, with the UPPERCASE labels as the durable fallback. | Realizes Phase 1 clarification asks and other discrete decision points. The ask degrades to prose with no contract impact — the contract names the *operation*, not the primitive. | Phrase multi-option forks so they read cleanly as prose. Update this row if a Codex structured prompt is observed — absence here reflects the documented surface, not a vendor guarantee. |
...
| **Unattended posture (`--unattended`)** | Trailing `--unattended` (no short alias) on `/ft-task`, `/ft-micro-task`, `/ft-goal-task`, and `/ft-close-epic`. The flag and the shared `unattended-mode.md` lazy fragment live in the loaded skill body. | Operator-less posture — a strict `--fast` superset (never pass both). Everything `--fast` suppresses stays suppressed, and on top of that the five gates a run with no operator cannot answer (🛠️ drift carve-out, destructive-action escalation, prerequisite ✋ ACTION, Step 1.5 concrete-model STOP, queued bundled in-📦 prompt) **park** the tasknote — `status: blocked` plus a closed-set `park-reason:` code — instead of firing a banner into an empty session. Removes pauses, never proof: the paper-complete guard is untouched. On `/ft-close-epic` it is not a `--fast` superset (that skill never took one) and the parent-flip is **deferred** rather than parked — the `.N` audit closes and commits, the parent line stays open, and the cohort move stays operator-owned. | An orchestrator or headless session driving a flowtron task with nobody watching. Skip it when you are present — `--fast` already covers "do not ask me", and a park costs a resume. |
