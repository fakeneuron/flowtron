...
| `SPEC/plan-filing.md` | the trigger line; §"Filing commits" (the four filing motions + the execution-skill fence); §"`## Completed` archive convention" (`/ft-audit*` trivial-fix carve-out, `/ft-close-epic` placement rule); §"`## Completed` rotation" (`/ft-stats` + `/ft-audit` as history consumers) — split out of the row above by [[CORE-595]] | `/ft-task`, `/ft-micro-task`, `/ft-file-followup`, `/ft-epic-discovery`, `/ft-close-epic`, `/ft-release`, `/ft-audit*`, `/ft-refactor`, `/ft-new-project`, `/ft-update`, `/ft-stats` | Canonical flowtron skill names. |
| `SPEC.md` + `SPEC/purpose-blurb.md` + `SPEC/gates.md` + `SPEC/cue-vocabulary.md` + `SPEC/gate-discipline.md` | §"Task-line format", §"Operator-gate cues", §"📝 Phase 1: Discovery", §"🧪 Phase 3: Testing & Linting", §"Post-closure protocol" (5 sites in `SPEC.md` — [[CORE-535.3]] brought it from 6 to 4 by moving the purpose blurb and the loop-tasks summary out of the core spec; [[CORE-536]] added §"Task-line format", whose `[unattended]` row names the flag the marker implies); the whole of `SPEC/purpose-blurb.md` (1 site); §"Operator-gate cues", §"Destructive-action escalation", §"Phase 1→2 exit gate", §"Conditional skip rule", §"Flag precedence and surface matrix" (+ its two subsections), §"`--fast` operator override", §"`--unattended` operator posture", §"What is inherited, and what is not", §"Park conversions", §"What `--unattended` never relaxes", §"`/ft-close-epic` under the posture", §"Gate discipline — read before skipping a gate" (14 sites in `SPEC/gates.md`, same any-mention counting rule as the `SPEC.md` half — [[CORE-532]], following [[CORE-527]]); §"Glyph layers and reuse", §"Emphasized inline ask shape", §"Accepted gate replies" (3 sites in `SPEC/cue-vocabulary.md`); §"Rationalizations", §"Red Flags", §"Refused carve-outs" (3 sites in `SPEC/gate-discipline.md`) — the last two groups are the vocabulary and discipline prose [[CORE-535.5]] split out of `SPEC/gates.md`; the gates.md count is unchanged at 14 because the split removed four mentioning sections and the matrix added four | `` `--fast` / `-f` `` operator force-skip flag | Operator force-skip mechanism implemented in the Claude Code skill bundle. Concept (operator force-skip) is platform-neutral; flag syntax is wiring-layer detail. SPEC documents the operator UX inline rather than punting to SKILL.md for density. |
