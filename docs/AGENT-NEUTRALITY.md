...
| `SPEC/tasknote-selection.md` | §"When to use a tasknote (and when not to)" (`/ft-spec` at intro + body; `/ft-refactor` bullet block); §"`## Completed` archive convention" (`/ft-audit*` trivial-fix carve-out) | `/ft-task`, `/ft-spec`, `/ft-refactor`, `/ft-starter-task`, `/ft-micro-task`, `/ft-file-followup`, `/ft-epic-discovery`, `/ft-close-epic`, `/ft-release`, `/ft-worktree-{start,end}`, `/ft-audit*` | Canonical flowtron skill names. |
