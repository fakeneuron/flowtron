...
| `SPEC/tasknote-selection.md` | §"When to use a tasknote (and when not to)" (`/ft-refactor` bullet block); §"`## Completed` archive convention" (`/ft-audit*` trivial-fix carve-out); §"`## Completed` rotation" (`/ft-stats` + `/ft-audit` as history consumers) | `/ft-task`, `/ft-refactor`, `/ft-micro-task`, `/ft-file-followup`, `/ft-epic-discovery`, `/ft-close-epic`, `/ft-release`, `/ft-audit*`, `/ft-stats` | Canonical flowtron skill names. |
