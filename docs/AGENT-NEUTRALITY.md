...
| `SPEC.md` + `SPEC/gates.md` | §"Operator-gate cues", §"🎯 Purpose blurb", §"📝 Phase 1: Discovery", §"🧪 Phase 3: Testing & Linting", §"🚀 Phase 4: Closure", §"Loop tasks", §"Post-closure protocol" (7 sites in `SPEC.md`); §"Phase 1→2 exit gate", §"`--fast` operator override", §"Conditional skip rule" (3 sites in `SPEC/gates.md`) | `` `--fast` / `-f` `` operator force-skip flag | Operator force-skip mechanism implemented in the Claude Code skill bundle. Concept (operator force-skip) is platform-neutral; flag syntax is wiring-layer detail. SPEC documents the operator UX inline rather than punting to SKILL.md for density. |
