...
| `SPEC/tasknote-selection.md` | §"When to use a tasknote (and when not to)" | `/ft-task`, `/ft-starter-task`, `/ft-micro-task`, `/ft-file-followup`, `/ft-debug`, `/ft-epic-discovery`, `/ft-close-epic`, `/ft-release`, `/ft-worktree-{start,end}` | Canonical flowtron skill names. |
