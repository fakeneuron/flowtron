...
| `SPEC/tasknote-selection.md` | §"When to use a tasknote (and when not to)" (`/ft-spec` at intro + body); §"`## Completed` archive convention" (`/ft-audit*` trivial-fix carve-out) | `/ft-task`, `/ft-spec`, `/ft-starter-task`, `/ft-micro-task`, `/ft-sidequest`, `/ft-file-followup`, `/ft-epic-discovery`, `/ft-close-epic`, `/ft-release`, `/ft-worktree-{start,end}`, `/ft-audit*` | Canonical flowtron skill names. |
