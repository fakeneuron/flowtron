...
| `SPEC/model.md` | (recommended-set paragraph + "Practical guidance and agent-aware defaults" subsection) | Primary labels `[heavy]` / `[medium]` / `[light]` + `opus`/`sonnet`/`grok` as examples | Per [[CORE-206]] (vocabulary) + [[CORE-207]] (practical examples, cross-provider calibration) + [[CORE-259]] (added the `[medium]` rung; Grok/Sonnet calibrate medium-tier) — neutral primary vocabulary + agent-aware usage guidance. Specific model names remain valid examples only. |
