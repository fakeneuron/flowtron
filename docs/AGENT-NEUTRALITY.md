...
| `SPEC.md` | §"Operator-gate cues", §"📝 Phase 1: Discovery", §"🧪 Phase 3: Testing & Linting", §"Conditional skip rule" (4 sites) | `` `--fast` / `-f` `` operator force-skip flag | Operator force-skip mechanism implemented in the Claude Code skill bundle. Concept (operator force-skip) is platform-neutral; flag syntax is wiring-layer detail. SPEC documents the operator UX inline rather than punting to SKILL.md for density. |
