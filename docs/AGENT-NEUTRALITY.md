...
| `SPEC/model.md` | (recommended-set paragraph + "Practical guidance and agent-aware defaults" subsection) | Primary labels `[heavy]` / `[light]` + `opus`/`sonnet`/`grok` as examples | Per [[CORE-206]] (vocabulary) + [[CORE-207]] (practical examples, Grok `[light]` default note, cross-provider calibration) — neutral primary vocabulary + agent-aware usage guidance. Specific model names remain valid examples only. |
