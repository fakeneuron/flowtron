...
Flowtron ships a `.github/workflows/ci.yml` CI pipeline that runs on push and pull request to `main`. The workflow reuses [AGENTS.md](../AGENTS.md) §"Validation" verbatim — `npm --prefix viz test`, `npm --prefix viz run typecheck`, `npm --prefix viz run lint`, `node --test tools/update-adopters.test.mjs`, `node --check tools/update-adopters.test.mjs`, and `node --check tools/update-adopters.mjs` — on Node 20 with npm caching.
