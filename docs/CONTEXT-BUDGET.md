...
| `SPEC/procedures/ft-task.md` | 38,000 | The always-loaded runner body for Codex/Cursor/Grok tasks — this project's non-Claude equivalent of `claude/skills/ft-task/SKILL.md`, read in full on every such task's cold start. Previously fell under this table's "not loaded to run an ordinary task" exemption, which reasoned from Claude's own load path (where `SKILL.md` is the loaded surface) rather than the full set of runners this repo supports. Budgeted at 38,000 by [[CORE-608]], sized like its lazy-but-near-universal siblings: the file at 34,565 chars plus ~1.5 working units, the same sizing rule `gate-postures.md`'s row uses — accepted as pre-sized, with the unit (~2,290) borrowed rather than measured on this file. Re-measured by [[CORE-631.3]] from the file's own `git log` byte deltas: the three large jumps in its history — [[CORE-395]] +4,760 and [[CORE-496]] +1,977 were catch-up resyncs to the Claude skill, [[CORE-473.4]] +3,379 was the whole `--unattended` mode — predate per-commit propagation; across the 25 touching commits since [[CORE-496]], a substantial edit runs +524 to +892 ([[CORE-504]] +892 · [[CORE-571]] +865 · [[CORE-610.3]] +698 · [[CORE-606]] +524) and a routine one +1 to +420. At 35,933 the 2,067 of headroom is ~2.3 units at the band's high end, so the cap stands; what would consume it in one task is a new mode at [[CORE-473.4]]'s scale, which is the case §"Known over budget" exists for. |
...
Measured 2026-09-20 at v5.31.0, refreshed by [[CORE-642]] — the release cut,
...
(28,103) + `SPEC.md` (47,831) + `.flowtron/tasknote/README.md` (9,881) +
`templates/tasknote-template.md` (5,188) + `SPEC/gates.md` (20,796) +
`SPEC/epic.md` (6,127) = **120,446 chars**
(≈30k tokens; `.flowtron/PLAN.md`'s own band, below, is excluded since it isn't
a flowtron-shipped surface). Measured at v5.31.0 from the same rows as the
tables below. Tracks
...
fixtures pointers (+566); unchanged at v5.31.0 ([[CORE-642]]).
...
|---|---|
| `SPEC.md` | 47,831 |
| `claude/skills/ft-task/SKILL.md` | 28,103 |
| `AGENTS.md` (`CLAUDE.md` is a symlink to it) | 7,396 |
| `.flowtron/tasknote/README.md` | 9,881 |
...
`procedures/ft-task.md` 35,933 · `gates.md` 20,796 ·
`gate-postures.md` 20,412 · `gate-discipline.md` 16,077 ·
`plan-filing.md` 15,991 · `tasknote-selection.md` 15,217 ·
`cue-vocabulary.md` 15,135 · `model.md` 14,636 ·
`unattended-candidacy.md` 12,277 · `blocked.md` 12,100 ·
`plan-parser.md` 9,241 · `loop.md` 8,074 · `post-closure.md` 7,823 ·
`epic.md` 6,127 · `procedures/README.md` 5,970 ·
`layout.md` 5,396 · `scope-boundaries.md` 5,347 ·
...
ft-release 32,803 · ft-epic-discovery 29,174 · ft-task 28,103 · ft-close-epic
27,914 · ft-audit 27,165 · ft-file-followup 26,177 · ft-micro-task 21,319 ·
ft-update 16,410 · ft-refactor 16,042 · ft-new-project 12,969 · ft-seed 12,313 ·
ft-audit-repo 9,886.
...
`ft-release` 113,697, `ft-task` 74,650. Splitting a body into fragments defers
