...
- [ ] **CORE-EPIC-438** [heavy]🧠 | cursor-wiring — Promote Cursor from contract-only stub to a first-class wiring platform: ship a `cursor/` sibling bundle per `docs/PLATFORMS.md` §"symmetric plug-in pattern", integrate it into adopter + release surfaces, then dogfood a real session to confirm it works (filed via /ft-epic-discovery; refined at .1 closure).
  ...
  - [ ] **CORE-438.2** [medium]🧩 | cursor-install-verify — Empirically verify Cursor skill discovery before anything is built on it: does a relative symlink from `.cursor/skills/` into `.flowtron/core/claude/skills/` resolve and invoke? Test the `.agents/skills/` + `.claude/skills/` compat paths too. Deliverable: locked install mechanism (symlink or copy) plus findings feeding .3.
  - [ ] **CORE-438.3** [heavy]🧠 | cursor-wiring-bundle — Ship the `cursor/` sibling dir per docs/PLATFORMS.md §"symmetric plug-in pattern": `AGENTS-snippet.md` (paste-block reference, install commands from .2's mechanism under a `One-time symlink wiring` heading so the fleet updater flags bumps, pinning notes) plus `procedures/ft-task.md` pointer. No skill wrappers — adopters wire canonical `claude/skills/` bodies.
