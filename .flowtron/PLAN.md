...
- [ ] **FE-EPIC-94** [heavy]🧠 | viz-app-decompose — Decompose `viz/src/ui/App.tsx` (541 lines, six responsibilities in one component) into pure selectors, extracted presentation, and state hooks — behavior-preserving, one seam per child, standing on the ~70-test App-level suite (filed via /ft-refactor; plan on the child starters).
  - [ ] **FE-94.2** [light]🔧 | app-characterization-gaps — Pin the seams later children move that the suite misses: the list/board toggle buttons (click → mode switch → `writeStoredViewMode`; both board tests seed localStorage instead), `listViewEmptySections` copy, the "No matches" empty state, the header starter suffix, and the error banner.
  - [ ] **FE-94.3** [medium]🧩 | app-derivation-extract — Lift the derived selectors (`matchesFilter`, child-pruning `filteredNodes`, `bySection` critical sort, `visibleIds`, `epicIds`, the four header counts) out of inline `useMemo`s into a pure `viz/src/ui/taskView.ts` with direct unit tests; App keeps the memo call sites.
  - [ ] **FE-94.4** [light]🔧 | app-diagnostics-extract — Collapse the four header warning badges (identical className, differing only in copy) and the three amber diagnostic strips (unparsed / duplicate-epic / near-miss heading) into `HeaderBadge` + `DiagnosticBanner` components. Pure JSX move, no state touched.
