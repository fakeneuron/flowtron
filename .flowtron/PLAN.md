...
- [ ] **CORE-590** [light] [unattended] | glossary-unattended-entry — `docs/GLOSSARY.md` carries per-segment entries for `[!critical]` and `[model]` but none for `[unattended]` (declined at CORE-494 under the Maintenance clause). Add the entry pointing at SPEC §"Task-line format" and `SPEC/unattended-candidacy.md`, extend the `grammar elements` list, and refresh the Maintenance last-update line.
