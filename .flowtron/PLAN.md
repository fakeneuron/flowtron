...
- [ ] **CORE-EPIC-631** [heavy]🧠 | budget-headroom — Three budgeted surfaces sit under one working unit of headroom by their own rows' sizing: `claude/skills/ft-release/**` at 122,895 / 125,000 (unit +4,000–5,300; capped this morning by CORE-622.2, then CORE-623/624/625 consumed ~4.9k the same day), `SPEC/procedures/ft-task.md` at 35,933 / 38,000, `SPEC/gate-postures.md` at 20,412 / 23,000. The ledger's own rule says trim or extract before raising; the big one has a structural cause — ten §7.1 pairs (A, B, C, H, J, M, N, O, P, Q) now carry their shell twice, in `step-7.1-mirror-pairs.md` (54,996) and again as the executable copy in `ci.yml`. Discovery supplied by audit-repo 2026-09-20. Surfaced by audit-repo 2026-09-20 (Theme: ratchets everywhere — and two are one edit from tripping).
  - [ ] **CORE-631.2** [heavy]🧠 | lifted-pairs-single-body — For the ten §7.1 pairs already lifted into the CI `drift` job, decide which copy is canonical and collapse the other to a pointer: either §7.1 keeps the shell and `ci.yml` stays the bound copy (Pair L as today), or `ci.yml` becomes the body and §7.1's pair entry shrinks to purpose + reads-set + a CI step name (Pair L's arrow flips). Land `claude/skills/ft-release/**` at ≥ 1.5 units under 125,000 without raising the cap; update `docs/CONVENTIONS.md` §"GitHub Actions CI" and the Pair L mapping rows to match.
  - [ ] **CORE-631.3** [medium]🧩 | procedures-postures-headroom — Re-measure the working unit for `SPEC/procedures/ft-task.md` and `SPEC/gate-postures.md` against their touching commits since each row was set; for each, either trim to ≥ 1.5 units under cap or record a justified raise in its `docs/CONTEXT-BUDGET.md` row, the way CORE-555 / CORE-558.5 did.
