...
- [ ] **CORE-EPIC-574** [heavy]🧠 | drift-ci-and-budget-headroom — Lift the mechanical remainder of `/ft-release` §7.1 into the CI `drift` job and restore ≥ one measured working unit (~3,400 chars) of headroom on every budgeted surface by trimming or lazy-fragmenting, never by raising a cap. `claude/skills/ft-task/SKILL.md` sits 1,161 under its 33,000 cap (third recurrence of the CORE-555 / CORE-558.5 defect); SPEC.md +1,903 and ft-task +2,994 since the v5.26.0 ledger. Discovery supplied by audit-repo 2026-09-11. Surfaced by audit-repo 2026-09-11 (Theme: The ratchet only loosens / Mechanical checks still walked by hand).
  - [ ] **CORE-574.2** [light]🔧 | ci-context-budget-check — Add a `drift` step that `wc -c`s every surface in `docs/CONTEXT-BUDGET.md` §"Budgets" against that table (numbers derived from the doc, never restated in the workflow); amend §"How this is enforced" (currently "No script, hook, or CI job runs this"); extend §7.1 Pair L's read-set to bind the lifted copy.
  - [ ] **CORE-574.3** [light]🔧 | ci-pairs-j-m — Lift §7.1 Pair J (command-stub `argument-hint:` ↔ documented flags) and Pair M (skill `description:` ↔ stub `argument-hint:`) into the `drift` job; update `docs/CONVENTIONS.md` §"GitHub Actions CI" roster of lifted checks.
...
- [ ] **CORE-EPIC-577** [heavy]🧠 | unattended-candidacy — Make every filing surface propose `[unattended]` candidates as part of its regular pass — attended: inside the existing confirm gate, operator's confirmation is the act; `--fast`/`--unattended`: emit `unattended-candidates:` and write nothing. Preserves "flowtron never writes it on its own discretion"; no autonomy chain. Conservative predicate, per-surface children (filed via /ft-epic-discovery; refined at .1 closure).
  ...
  - [ ] **CORE-577.2** [heavy]🧠 | candidacy-contract — New lazy `SPEC/unattended-candidacy.md`: recommend-never-write rule, three postures (attended → inside the existing confirm gate; `--fast`/`--unattended` → `unattended-candidates:` line, zero writes), the conservative predicate, the Final-Summary persistence hook. `SPEC.md` §"Task-line format" gets a one-paragraph pointer; `docs/EXTERNAL-AGENTS.md` step 2 cites it. CI `drift` step: every filer that Reads the module carries the `unattended-candidates:` literal + labeled mirror.
  - [ ] **CORE-577.3** [medium]🧩 | epic-discovery-candidacy — `/ft-epic-discovery` Step 7 runs the predicate over `.2..M+1` and the `.N` row, folds candidates into the existing reconcile review prompt, writes the token only on confirmed rows after `[model]`. Step 4's parent + `.1` lines are never proposed. Labeled mirror to `SPEC/unattended-candidacy.md`.
  - [ ] **CORE-577.4** [heavy]🧠 | file-followup-candidacy — `/ft-file-followup` default + `--starter`: predicate over the drafted row, proposal inside the Step 3 / S3 review gate, token only on confirm. `--park`: no candidacy. `--unattended`: `unattended-candidates:` in the Step 5 report, zero writes, echoed into the discharging runner's Final Summary (runner closure fragments updated).
...
- [ ] **CORE-EPIC-575** [medium]🧩 | viz-dependency-posture — Make `npm --prefix viz audit` a signal again and settle the gray-matter → js-yaml 3.x residue. Audit reports 5 advisories (2 high): vitest 4.1.6 (@vitest/mocker GHSA-82fw-gwwq-j7x9), browserslist ×2, baseline-browser-mapping, and js-yaml 3 via gray-matter — the last already neutralised in `viz/src/tasknote-parse.ts` by the engine override but still in the tree, so audit stays permanently red. No audit gate in CI, no Dependabot. Discovery supplied by audit-repo 2026-09-11. Surfaced by audit-repo 2026-09-11 (Theme: Dependency hygiene is a blind spot).
  - [ ] **CORE-575.2** [light]🔧 | npm-audit-fix-in-range — `npm audit fix` for the in-range set (vitest / browserslist / baseline-browser-mapping); rerun the full AGENTS.md §"Validation" gates.
