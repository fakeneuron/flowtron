...
- [ ] **CORE-EPIC-389** [heavy]🧠 | audit-family-consolidation — merge the six forked audit scaffolds into one parameterized `/ft-audit <domain>` + six-file pass library per the [[CORE-287]] overlay precedent; retire the five siblings (claude + codex + command wrappers); rewrite MIGRATION §1.2.1 with an adopter migration note for existing forks. Verdict source: [[CORE-388]]. Promoted from plain CORE-389 (refined at .1 closure).
  ...
  - [ ] **CORE-389.2** [heavy]🧠 | parameterized-survivor — rewrite `claude/skills/ft-audit/SKILL.md` as the `/ft-audit <domain>` dispatcher over a six-file `passes/` library (per-domain passes, severity guide, scope/rubric hints, specialist hard rules); default domain general; §0 forker checklist survives once; repoint `/ft-release` §7.1's subroutine to `/ft-audit docs`.
  - [ ] **CORE-389.3** [light]🔧 | sibling-retirement — delete the five retired scaffolds (`claude/skills/ft-audit-{backend,frontend,security,performance,docs}/`), their `claude/commands` wrappers, and codex mirrors; rewrite the surviving command + codex wrappers for the parameterized form; update roster enumerations (ft-flowtron, SPEC §"Skill namespace") and count mirrors (26→21).
