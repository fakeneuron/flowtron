...
- [ ] **CORE-386** [medium]🧩 | skill-rationalizations-redflags — adopt two sections from the agent-skills anatomy (Overview / When to Use / Process / **Rationalizations** / **Red Flags** / Verification): Rationalizations enumerates the excuses an agent makes to skip a step, Red Flags names the observable symptoms. Scope deliberately narrow — `SPEC/gates.md` plus the consolidated `/ft-audit` skill (post-[[CORE-389]]), i.e. the surfaces whose whole job is resisting shortcut-taking. Explicitly **not** a retrofit across all 26 skills. CORE-388 coordination resolved: target one audit skill, not eight.
- [ ] **CORE-387** [medium]🧩 | cross-artifact-consistency — no surface today checks whether an *active* tasknote's plan still matches `PLAN.md` and SPEC; `ft-audit-docs` covers shipped-doc drift only, and the Relevance Assessment tests staleness by judgment rather than by cross-reference. Peer precedent: spec-kit's `/speckit.analyze`. Extension-first per VISION §principles — fold into `ft-audit-docs` as a pass, or into the Phase 2 gate, rather than adding a skill to a 26-skill roster.
- [ ] **CORE-389** [heavy]🧠 | audit-family-consolidation — merge the six forked audit scaffolds into one parameterized `/ft-audit <domain>` + pass library per the [[CORE-287]] overlay precedent; ship an adopter migration note for existing forks. Verdict source: [[CORE-388]]. Likely an epic on contact.
