...
- [ ] **CORE-504** [medium] | scaffold-purpose-blurb — `/ft-task` scaffolds and drops straight into Phase 1 Discovery with no statement of what the task is — the operator gets no immediate read. Emit a 1-2 sentence purpose blurb at scaffold, before Discovery. Scope question: `/ft-task` only, or the whole initiating cohort (`/ft-goal-task`, `/ft-micro-task`, epic skills) plus a SPEC line.
