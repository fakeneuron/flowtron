...
- [ ] **CORE-563** [medium]🧩 | filing-precheck-race — `/ft-file-followup` Step 4's pre-check reads `git status` before the append, so an operator save landing between the check and `git add` is invisible and gets swept into the agent's filing commit — observed 2026-09-10. Consider verifying the staged diff matches the appended line before committing. Surface: `SPEC/tasknote-selection.md` §"Filing commits".
- [ ] **CORE-564** [medium]🧩 | ft-task-area-resolution — Harden `/ft-task` Step 2's `<area>` resolution so an agent cannot derive the archive folder by lowercasing the task-ID prefix. Observed failure: in natabula (`NAT-` → `archive/natabula/`) agents have created a stray `archive/nat/` **three times** — NAT-089 (5 notes), NAT-097.2 (2 more, which also added an explicit ⚠️ README warning naming both prior occurrences), and NAT-220 (a third, *after* that warning). It is not untidiness: NAT-230 then ran its Phase 1 **Archive skim** against the misfiled `archive/nat/`, found it empty, and recorded "no prior archived tasknotes in this repo" against an archive holding 374 — so a wrong `<area>` silently voids the skim for the misfiling task itself, which is a correctness failure in Discovery, not a filing nit. Root cause is that Step 2 says *"Resolve the **Area** from the task ID prefix per SPEC §'Task ID convention'"* and only falls through to `.flowtron/tasknote/README.md` when the prefix is *unknown* — a `NAT-` prefix reads as known, so the adopter's own prefix→folder table is never consulted and lowercasing looks correct. Consider making the README table the **mandatory** lookup for `<area>` on any non-canonical prefix (or all prefixes), and stating that `<area>` is never derived from the ID. Surface: `claude/skills/ft-task/SKILL.md` Step 2 + the Step 4 Archive-skim bullet, the same two restatements in `ft-micro-task` / `ft-goal-task`, `SPEC.md` §"Task ID convention", and `SPEC/procedures/ft-task.md`. Adopter-side precedent for the mechanical form: natabula **NAT-232** added `scripts/check-archive-layout.sh`, which parses that README table and fails on both a non-canonical directory and a prefix/folder mismatch — that guard catches the misfile *after* the fact in one repo; this row is the upstream half that prevents it in every adopter. Routed from natabula **NAT-232** per `SPEC/scope-boundaries.md` §"Cross-repo edit remit".
