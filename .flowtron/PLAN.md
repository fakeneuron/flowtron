...
- [ ] **CORE-EPIC-604** [heavy] | gate-tiering-cold-start — Cut a flagless ft-task cold start (~172k chars of flowtron-owned reads) by tiering `SPEC/gates.md` flag-posture material into a lazy module, trimming runner, command-stub and `model.md` surfaces, and lowering the `## Completed` rotation bound. No constitution split, no INDEX file (filed via ft-epic-discovery; refined at .1 closure).
  ...
  - [ ] **CORE-604.2** [heavy] | gate-postures-split — Move the flag-posture material (`--fast` override, `--unattended` posture with its park conversions and close-epic section, flag precedence + surface matrix; ≈17.4k) from `SPEC/gates.md` into new `SPEC/gate-postures.md`, stub headings left so copied citations resolve. Wire the Read at the runners' flag and `[unattended]`-marker branches, `unattended-mode.md`, and ft-close-epic; repoint citations in SPEC.md, skills, SOP, adopter snippet; add a CONTEXT-BUDGET row.
  - [ ] **CORE-604.3** [medium] | runner-stub-model-trim — Fold the per-step `unattended-mode = true` clauses in the ft-task and ft-micro-task bodies into one pointer each at `unattended-mode.md` §"Conversion map"; cut `claude/commands/ft-task.md` to invoke + usage; put the four-row tier ladder into SPEC.md §"Model field"; move `model.md`'s calibration table to docs/PLATFORMS.md and drop Haiku. Cite-don't-restate; no operative imperative removed (CORE-558.4).
  - [ ] **CORE-604.4** [light] | rotation-bound-and-ledger — Lower the `## Completed` rotation advisory from 150 to 60 rows in SPEC/plan-filing.md and the runners' Step 1 check (mirror the SOP), then rotate flowtron's own Completed with operator-confirmed month blocks; add a "default-path cold start" sum line (stub + skill + SPEC + README + template + gates + plan-filing + epic) to docs/CONTEXT-BUDGET.md §Ledger — a ledger row, not a budget.
