...
- [ ] **CORE-382** [medium]🧩 | positioning-oversight-checkpoints — reframe the outward pitch from context-economy ("keep AI context windows small") to agent-oversight checkpoints ("where you catch the agent before it wastes a session"): the 4 phases + Relevance Assessment + Acceptance criteria are already the checkpoints, they're just sold as ceremony. Requires editing `docs/VISION.md` §"Who it's for" — the `no PR review queue` line (VISION.md:11) is what currently rules this framing out. Peer precedent: Backlog.md leads on spec/plan/code review checkpoints. Landed 2026-07-31 from a competitive-landscape review; operator signed off on moving the VISION audience line.
- [ ] **CORE-383** [medium]🧩 | readme-above-fold — restructure `README.md` so the pitch precedes the index: 2-line hook (from CORE-382) → `LOGO.png` (present at repo root, currently unreferenced anywhere in README) → a `viz/` screenshot → 60-second quickstart → doc index collapsed into `<details>`. Today `## Documents` runs lines 10–54 and the quickstart doesn't appear until line 55. Add shields (license, latest tag) and the dogfooding stat sourced from `/ft-stats` — flowtron's own completed-task volume is a claim no peer can fake. Peer precedent: Backlog.md (demo GIF under the headline), spec-kit (logo → tagline → 7-step Get Started).
- [ ] **CORE-384** [light]🔧 | conventions-declines-distribution — add four `docs/CONVENTIONS.md` §"Declines" entries recording positions currently held but unwritten: (1) MCP server — task-master ships 36 tools ≈ 21k tokens, contradicting SPEC #3 one-task-per-context-window; (2) CLI / daemon distribution — contradicts SPEC #2 zero scripts, and is the shape both spec-kit and Backlog.md take; (3) preset / extension / bundle override stacking — spec-kit resolves templates through a 4-level priority chain, flowtron declines the indirection; (4) non-submodule distribution — the submodule *is* the SPEC #5 version-pin mechanism, and skills resolve `<root>` at `.flowtron/core/` (`claude/skills/ft-task/SKILL.md:16`), so a plugin/marketplace install would deliver inert skills with no contract to read.
- [ ] **CORE-385** [light]🔧 | skill-trigger-frontmatter — consistency sweep over shipped skill `description:` frontmatter. The `ft-audit-*` family encodes explicit trigger conditions ("Use when the user asks to…"); `ft-task`, `ft-quality`, `ft-stats` and peers state only what they do, so they never auto-surface. Reference pattern already in-repo; extension-first, no new shape.
- [ ] **CORE-386** [medium]🧩 | skill-rationalizations-redflags — adopt two sections from the agent-skills anatomy (Overview / When to Use / Process / **Rationalizations** / **Red Flags** / Verification): Rationalizations enumerates the excuses an agent makes to skip a step, Red Flags names the observable symptoms. Scope deliberately narrow — `SPEC/gates.md` plus the `ft-audit-*` family, i.e. the surfaces whose whole job is resisting shortcut-taking. Explicitly **not** a retrofit across all 25 skills. Coordinate with CORE-388 so the sections aren't written into skills that consolidation may remove.
- [ ] **CORE-387** [medium]🧩 | cross-artifact-consistency — no surface today checks whether an *active* tasknote's plan still matches `PLAN.md` and SPEC; `ft-audit-docs` covers shipped-doc drift only, and the Relevance Assessment tests staleness by judgment rather than by cross-reference. Peer precedent: spec-kit's `/speckit.analyze`. Extension-first per VISION §principles — fold into `ft-audit-docs` as a pass, or into the Phase 2 gate, rather than adding a skill to a 25-skill roster.
- [ ] **CORE-388** [heavy]🧠 | skill-roster-utility-review — deep dive on whether all 25 shipped skills earn their place; likely promotes to an epic on contact. Baseline for comparison: spec-kit exposes ~8 commands at ~115k stars. Preliminary signal — 15 of the 25 sit in two clusters that may consolidate. (a) Seven task-entry skills (`ft-task`, `ft-starter-task`, `ft-micro-task`, `ft-sidequest`, `ft-file-followup`, `ft-goal-task`, `ft-debug`) differ mainly by task size and framing; `SPEC/tasknote-selection.md` is 235 lines existing purely to disambiguate them, which is itself the smell — needing a long doc to choose an entry point means the entry points aren't self-evident. (b) Eight `ft-audit-*` skills are each self-described as a "stack-neutral scaffold" adopters fork and fill in, i.e. templates over a shared 5-pass shape rather than eight distinct workflows — candidate for one parameterized skill plus a pass library. Deliverable: keep / merge / cut verdict per skill with rationale, not a rewrite.
