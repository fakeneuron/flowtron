...
3. Insert tickets in the correct priority section — `## High` / `## Medium` / `## Low` for blocking improvements; `## Future Opportunities` for non-blocking ideas. Use the `[!critical]` task-line flag for urgent items — there is no `## Critical` section (retired per SPEC §"Priority levels"). Add a `Surfaced by audit YYYY-MM-DD (Finding #N, <severity>)` parenthetical to each ticket's description so future-you can trace a ticket back to its origin without re-running the audit.
