...
This SKILL.md is a fork of the stack-neutral scaffold at `claude/skills/audit-docs/SKILL.md`, customized for flowtron's own doc set per the §1.2.1 fork-not-symlink contract. The fork is yours — flowtron version bumps do not touch it.

## Invocation modes

- **Standalone** (`/audit-docs [scope]`) — present the full report (Summary, Exploratory Insights, Proposed tasks, Questions) and write tickets into `_project/PLAN.md` per §5.
- **Subroutine** (called by `/release` §7.1) — receive an explicit scope from the caller, present the report inline (no `AskUserQuestion` for non-blockers), surface drift back to `/release` for inline fix-up rather than filing tickets. Skip §5 in this mode.
...
1. **Resolve scope** from `$ARGUMENTS`:
   - `all` or empty → the AI-referenced docs set declared in `_project/tasknote/README.md` §"AI-referenced docs":
     ...
     ```
   - `broad` → the full markdown contract surface (matches the broader `/audit` skill's scope):
     ```text
     SPEC.md
     SPEC/**/*.md
     claude/skills/**/*.md
...
**Subroutine mode (called by `/release`):** skip ticket writing entirely. Instead, surface the report and any "fix now during release cut" recommendations back to the caller for inline handling in release Phase 2 / 7.1. The release skill decides whether to absorb the fix into the current cut or file followups.

## 5. Write the proposed tasks into `_project/PLAN.md` (standalone mode only)

Skip this entire section when invoked as a subroutine by `/release`.
...
- **Subroutine mode skips ticket writing.** `/release` is the orchestrator — it decides which findings to fix in the current cut.
