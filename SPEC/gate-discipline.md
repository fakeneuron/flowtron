...
- You are constructing an argument for why `--fast` covers a case that
  [`SPEC/gate-postures.md`](gate-postures.md) §"`--fast` operator override" does not list.
...
- You are constructing an argument for why `--unattended` covers a gate the
  ...
  [`SPEC/gate-postures.md`](gate-postures.md) §"Flag precedence and surface matrix" already
  refuses.
...
precisely so it is findable; [`SPEC/gate-postures.md`](gate-postures.md) §"Park conversions"
